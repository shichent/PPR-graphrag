...
class KTBuilder:
    ...
    def _merge_and_build_graph(self):
        from collections import defaultdict

        # Pass 1: collect entity info keyed by name — only first appearance chunk id
        entity_chunk_ids = {}  # name -> first chunk_id
        entity_type_map = {}

        for ext in self.all_extractions:
            cid = ext['chunk_id']
            for name in ext['attributes']:
                if name not in entity_chunk_ids:
                    entity_chunk_ids[name] = cid
            for triple in ext['triples']:
                if len(triple) >= 3:
                    for name in (triple[0], triple[2]):
                        if name not in entity_chunk_ids:
                            entity_chunk_ids[name] = cid
            for name, etype in ext['entity_types'].items():
                entity_type_map[name] = etype

        # Pass 2: add entity nodes
        entity_to_node = {}
        counter = 0
        for name, first_chunk_id in entity_chunk_ids.items():
            nid = f"entity_{counter}"; counter += 1
            props = {"name": name, "chunk id": first_chunk_id}
            if name in entity_type_map:
                props["schema_type"] = entity_type_map[name]
            self.graph.add_node(nid, label="entity", properties=props, level=2)
            entity_to_node[name] = nid

        # Pass 3: attribute nodes, dedup by (entity_name, attr_value)
        # Nodes store first chunk id only; edges store all chunk ids
        attr_first_chunk_id = {}        # key -> first chunk_id (for node property)
        attr_all_chunk_ids = defaultdict(list)  # key -> all chunk_ids (for edge)
        for ext in self.all_extractions:
            cid = ext['chunk_id']
            for entity_name, attrs in ext['attributes'].items():
                for attr in attrs:
                    key = (entity_name, attr)
                    if key not in attr_first_chunk_id:
                        attr_first_chunk_id[key] = cid
                    if cid not in attr_all_chunk_ids[key]:
                        attr_all_chunk_ids[key].append(cid)

        for (entity_name, attr), first_cid in attr_first_chunk_id.items():
            if entity_name not in entity_to_node:
                continue
            anid = f"attr_{counter}"; counter += 1
            self.graph.add_node(anid, label="attribute",
                                properties={"name": attr, "chunk id": first_cid}, level=1)
            self.graph.add_edge(entity_to_node[entity_name], anid,
                                relation="has_attribute", chunk_id=attr_all_chunk_ids[(entity_name, attr)])

        # Pass 4: triple edges, dedup by (subj_name, pred, obj_name)
        triple_chunk_ids = defaultdict(list)
        for ext in self.all_extractions:
            cid = ext['chunk_id']
            for triple in ext['triples']:
                validated = self._validate_triple_format(triple)
                if not validated:
                    continue
                subj, pred, obj = validated
                key = (subj, pred, obj)
                if cid not in triple_chunk_ids[key]:
                    triple_chunk_ids[key].append(cid)

        for (subj, pred, obj), chunk_ids in triple_chunk_ids.items():
            if subj not in entity_to_node or obj not in entity_to_node:
                continue
            self.graph.add_edge(entity_to_node[subj], entity_to_node[obj],
                                relation=pred, chunk_id=chunk_ids)

        self.node_counter = counter
        logger.info(
            f"Graph built: {len(entity_to_node)} entities, "
            f"{len(attr_first_chunk_id)} attributes, {len(triple_chunk_ids)} unique triples"
        )
