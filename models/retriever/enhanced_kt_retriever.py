...
class KTRetriever:
    def __init__(
        ...
        self.top_k_graph = config.retrieval.top_k_graph if config else 100
        self.top_k_chunks = config.retrieval.top_k_chunks if config else 20
        self.use_pagerank = config.retrieval.use_pagerank if config else False
        self.pagerank_alpha = config.retrieval.pagerank_alpha if config else 0.50
    ...
    def _parallel_dual_path_retrieval(self, question_embed: torch.Tensor, question: str) -> Dict:
        ...
        limited_chunk_ids = list(all_chunk_ids)[:self.top_k_chunks]
    ...
    def _execute_retrieval_strategies_parallel(self, question_embed: torch.Tensor, question: str, q_embed) -> Dict:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            faiss_node_future = executor.submit(
                self._faiss_node_search, q_embed, min(self.top_k_graph * 3, 300)
            )
            faiss_relation_future = executor.submit(
                self._faiss_relation_search, q_embed, self.top_k_graph
            )
    ...
    def _node_relation_retrieval(self, question_embed: torch.Tensor, question: str = "") -> Dict:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            search_k = min(self.top_k_graph * 3, 300)
            ...
            future_pagerank = None
            if self.use_pagerank and question:
                future_pagerank = executor.submit(
                    self._personalized_pagerank,
                    future_keywords
                )
            ...
            pagerank_scores = {}
            if future_pagerank:
                pagerank_scores = future_pagerank.result()
                if pagerank_scores:
                    existing = {node for node, _ in candidate_nodes}
                    candidate_nodes = [
                        (node, sim + pagerank_scores.get(node, 0.0))
                        for node, sim in candidate_nodes
                    ]
                    n_ppr = int(math.sqrt(self.top_k_graph))
                    added = 0
                    for node, ppr_score in sorted(pagerank_scores.items(), key=lambda x: x[1], reverse=True):
                        if node not in existing:
                            candidate_nodes.append((node, ppr_score))
                            existing.add(node)
                            added += 1
                            if added >= n_ppr:
                                break
            ...
            top_nodes = [node for node, score in candidate_nodes[:self.top_k_graph] if score > 0.01]
    ...
    def _execute_faiss_relation_search(self, q_embed) -> List[str]:
        _, I_relations = self.faiss_retriever.relation_index.search(
            q_embed.reshape(1, -1), self.top_k_graph
        )
        return [
    ...
    def _personalized_pagerank(self, future_keywords) -> Dict[str, float]:
        if future_keywords is None:
            keywords = []
        elif hasattr(future_keywords, 'result'):
            keywords = future_keywords.result()
        else:
    ...
    def _format_scored_triples(self, scored_triples: List[Tuple[str, str, str, float]]) -> List[str]:
        ...
        for h, r, t, score in scored_triples:
            ...
            if not head_text or not tail_text or head_text.startswith(('[Error', '[Unknown')) or tail_text.startswith(('[Error', '[Unknown')):
                continue
    ...
    def _rank_chunk_ids_by_similarity(
        ...
        q = question_embed.cpu().numpy().astype('float32').flatten()
        scored = []
        for chunk_id in chunk_ids:
            idx = self.chunk_id_to_index.get(str(chunk_id))
            if idx is None:
                continue
            try:
    ...
    def _get_node_chunk_id(self, node_data: dict) -> list:
        """Extract chunk IDs from node data as a list, handling both old and new structures."""
        if isinstance(node_data.get('properties'), dict):
            cid = node_data['properties'].get('chunk id')
        else:
            cid = node_data.get('chunk id')
        ...
        for node in nodes:
            try:
                if node in self.graph.nodes:
                    cids = self._get_node_chunk_id(self.graph.nodes[node])
                    if cids:
                        for cid in cids:
                            chunk_ids.add(str(cid))
                    else:
